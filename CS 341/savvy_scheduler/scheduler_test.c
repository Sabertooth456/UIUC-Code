...
/**
 * Utility functions to get globals
 */
...
/**
 * Prints out the usage of the tester
 */
static int usage(char *name) {
    fprintf(stderr, "%s <Any number from 1 to 10>\n", name);
    exit(13);
}
...
/**
 * This function will make and schedule a job with a specified runtime, priority
 * and arrival time
 */
...
static int test_new_job(void) {
    ...
    scheduler_quantum_expired(NULL, 1);
    scheduler_job_finished(j, 13);
    free(j);
...
static int test_comparer_fcfs(void) {
    ...
    scheduler_job_finished(j1, 11);
    scheduler_quantum_expired(NULL, 11);
    scheduler_job_finished(j2, 21);
...
static int test_comparer_sjf(void) {
    ...
    scheduler_quantum_expired(NULL, 2);
    // schedule job3
    ...
    scheduler_job_finished(j1, 2);
    scheduler_job_finished(j2, 5);
    scheduler_quantum_expired(NULL, 5);
    scheduler_job_finished(j3, 8);
...
static int test_comparer_pri(void) {
    ...
    scheduler_job_finished(j1, 10);
    scheduler_quantum_expired(NULL, 10);
    scheduler_job_finished(j2, 20);
    scheduler_quantum_expired(NULL, 20);
    scheduler_job_finished(j3, 30);
...
static int test_comparer_psrtf(void) {
    ...
    scheduler_job_finished(j2, 4);
    scheduler_quantum_expired(NULL, 4);
    scheduler_job_finished(j1, 13);
    scheduler_quantum_expired(NULL, 13);
    scheduler_job_finished(j3, 22);
