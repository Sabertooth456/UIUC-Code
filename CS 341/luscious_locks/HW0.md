...
int main() {
	...
	int fd = open("hello_world.txt", O_CREAT | O_RDWR | O_TRUNC, mode);
...
int main() {
	close(STDERR_FILENO);
	mode_t mode = S_IRUSR | S_IWUSR;
	int fd = open("errors.txt", O_CREAT | O_RDWR | O_TRUNC, mode);
	open("", 0);
	perror("");
	close(fd);
	return 0;
...
5. 'write()' lets you choose the output stream you right to, while printf can only write to a pre-determined file descriptor.
...
6. ptr is a character pointer to a string literal, which is located in read-only memory. Trying to change the string literal with a dereference is what causes the segfault.
7. 12
8. 5
9. char *X = "12"
10. int Y = 1;
