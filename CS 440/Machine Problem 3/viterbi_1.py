...
def training(sentences):
    ...
    total_words = sum(init_prob.values())
    for tag in init_prob.keys():
        init_prob[tag] /= total_words
    for tag in emit_prob.keys():
        tag_words = sum(emit_prob[tag].values())
        for word in emit_prob[tag].keys():
            if (tag_words > 0):
                emit_prob[tag][word] /= tag_words
            else:
                emit_prob[tag][word] = emit_epsilon
    for tag0 in trans_prob.keys():
        tag0_total = sum(trans_prob[tag].values())
        for tag1 in trans_prob[tag0].keys():
            if (tag0_total > 0):
                trans_prob[tag0][tag1] /= tag0_total
            else:
                trans_prob[tag0][tag1] = epsilon_for_pt
    
    print(init_prob, emit_prob, trans_prob)
