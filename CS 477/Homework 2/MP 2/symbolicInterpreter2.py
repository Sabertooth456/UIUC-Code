...
def executeSequence(pathCondition, store, command):
    # TODO
    if isinstance(command.s1, Ite):
        e1 = executeCommand(evaluateExpr(pathCondition, store, command.s1.cond), store, command.s1.s1)
        e2 = executeCommand(Not(evaluateExpr(pathCondition, store, command.s1.cond)), store, command.s1.s2)
    else:
...
def executeCommand(pathCondition, store, command):
    ...
    if isinstance(command, Skip):
        ...
        if (checkSat(evaluateExpr(pathCondition, store, command.cond))):
            return executeCommand(evaluateExpr(pathCondition, store, command.cond), store, command.s1)
        else:
            return executeCommand(evaluateExpr(Not(pathCondition), store, command.cond), store, command.s2)
    else:
        print('ERROR')
        return None