...
part1_code:
	...
	li $t4, 0x00040000
    sw $t4, POWERWASH_ON						# Near-infinite water is perfectly balanced
    
    li	$a0, 5
    jal move_east								# move_east(5)
    
    li	$a0, 15
    jal move_north								# move_north(16)
    
    li	$a0, 5
    jal move_west								# move_west(5)
    
    li	$a0, 5
    jal move_south								# move_south(5)
    
    li	$a0, 5
    jal move_west								# move_west(5)
    
    li	$a0, 17
    jal move_north								# move_north(15)
    
    li	$a0, 8
    jal move_east								# move_east(8)
    
    li	$a0, 5
    jal move_north								# move_north(5)
    
    li	$a0, 1
    jal move_east								# move_east(1)
    
    li	$a0, 50000
    jal wait									# wait(50000)
    
    li	$a0, 30
    jal move_east								# move_east(30)
    
    li	$t0, 1									# 1
    sw	$t0, POWERWASH_OFF						# POWERWASH_OFF = 1
...
move_absolute:								# Move 1 tile in the specified direction ($a0)
	sub $sp, $sp, 8								# Allocate 5 words of memory from the stack
	sw 	$ra, 0($sp)								# Store the $return address
	sw 	$a0, 4($sp)								# Store the initial $a0
	# lw	$t0, ANGLE								# $t0 = ANGLE
	# sw 	$t0, 8($sp)								# Store the initial ANGLE
	# lw	$t0, ANGLE_CONTROL						# $t0 = ANGLE_CONTROL
	# sw 	$t0, 12($sp)							# Store the initial ANGLE_CONTROL
	# lw	$t0, VELOCITY							# $t0 = VELOCITY
	# sw 	$t0, 16($sp)							# Store the initial VELOCITY
	
	move $t0, $a0								# $t0 = $a0 degrees
	sw 	$t0, ANGLE								# ANGLE = $a0 degrees
	li 	$t0, 1									# 1
	sw 	$t0, ANGLE_CONTROL						# ANGLE_CONTROL = absolute angle
	li 	$t0, 8									# 10 pixel per 10,000 cycles
	sw 	$t0, VELOCITY							# VELOCITY = 10 pixel per 10,000 cycles
	
	li	$a0, 10000								# $a0 = 10,000
	jal wait									# wait(1000)
	lw 	$a0, 4($sp)								# Restore the initial $a0
	...
	# lw	$t0, 16($sp)							# $t0 = initial VELOCITY
	# sw 	$t0, VELOCITY							# Restore the initial VELOCITY
	# lw	$t0, 12($sp)							# $t0 = intiial ANGLE_CONTROL
	# sw 	$t0, ANGLE_CONTROL						# Restore the initial ANGLE_CONTROL
	# lw	$t0, 8($sp)								# $t0 = initial ANGLE
	# sw 	$t0, ANGLE								# Restore the initial ANGLE
	lw	$a0, 4($sp)								# Restore the initial $a0
	lw 	$ra, 0($sp)								# Restore the $return address
	add $sp, $sp, 8								# Deallocate 5 words of memory from the stack
	jr	$ra										# return

move_steps:									# Move ($a1) tiles in the specified direction ($a0)
...
move_north:									# Move ($a0) tiles north
	sub $sp, $sp, 12							# Allocate 3 word(s) of memory from the stack
	sw	$ra, 0($sp)								# Store the $return address
	sw	$a0, 4($sp)								# Store the initial $a0
	sw	$a1, 8($sp)								# Store the initial $a1
...
move_south:									# Move ($a0) tiles south
	sub $sp, $sp, 12							# Allocate 3 word(s) of memory from the stack
	sw	$ra, 0($sp)								# Store the $return address
	sw	$a0, 4($sp)								# Store the initial $a0
	sw	$a1, 8($sp)								# Store the initial $a1
