...
eval expr@(Pair v1 v2) = case flattenList expr of
  ...
  Right lst -> evalList lst where
    ...
    evalList [Symbol "unquote", e] = throwError $ InvalidExpression e
    ...
    evalList [Symbol "lambda", Pair args body] =
      do argList <- getList args
         argNames <- mapM getSym argList
         env <- get
         return $ Func argNames body env
    evalList [Symbol "lambda", _] = throwError $ InvalidSpecialForm "lambda" (Symbol "lambda")
    ...
    -- to match the syntax
    evalList [Symbol "define", Symbol var, value] =
      do evalValue <- eval value
         modify $ H.insert var evalValue
         return Void
    evalList [Symbol "define", _, _] = throwError $ InvalidSpecialForm "define" (Symbol "define")
...
apply (Func params body closureEnv) args =
  do when (length params /= length args) $ throwError $ Unimplemented "Mismatched argument lengths"
