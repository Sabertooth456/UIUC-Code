                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty() && mergeQueue.empty()) {
        std::cout << "removeSmallest() : both queues are empty" << std::endl;
        return nullptr;
    } else if (singleQueue.empty()) {
        TreeNode* m_node = mergeQueue.front();
        mergeQueue.pop();
        return m_node;
    } else if (mergeQueue.empty()) {
        TreeNode* s_node = singleQueue.front();
        singleQueue.pop();
        return s_node;
    }
    TreeNode* s_node = singleQueue.front();
    TreeNode* m_node = mergeQueue.front();
    if (m_node->freq.getFrequency() < s_node->freq.getFrequency()) {
        mergeQueue.pop();
        return m_node;
    } else {
        singleQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (Frequency freq : frequencies) {
        singleQueue.push(new TreeNode(freq));
    }
    // While both queues have nodes or a queue has more than one object
    while ((!singleQueue.empty() && !mergeQueue.empty()) || (singleQueue.size() > 1 || mergeQueue.size() > 1)) {
        TreeNode* first_node = removeSmallest(singleQueue, mergeQueue);
        TreeNode* second_node = removeSmallest(singleQueue, mergeQueue);
        TreeNode* merged_node = new TreeNode(first_node->freq.getFrequency() + second_node->freq.getFrequency());
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        if (current == nullptr) {
            std::cout << "decode() : invalid bfile" << std::endl;
            return;
        }

        bool cur_bit = bfile.getNextBit();
        // If it's a leaf node, print its character to stringstream
        if (cur_bit == 0) {
            current = current->left;
        } else if (cur_bit == 1) {
            current = current->right;
        } else {
            std::cout << "decode() : impossible else branch reached" << std::endl;
            return;
        }
...
void HuffmanTree::writeTree(TreeNode* current, BinaryFileWriter& bfile)
{
    ...
    // Impossible case
    if (current == nullptr) {
        std::cout << "writeTree() : current == nullptr" << std::endl;
        return;
    } else if (current->left == nullptr && current->right == nullptr) { // Leaf node
        bfile.writeBit(1);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits()) {
        ...
        readTree(root_, bfile);
    } else {
        root_ = nullptr;
        std::cout << "readTree() : initial bfile is empty" << std::endl;
    }
    print(std::cout);
    return root_;
}
...
/**
 * If bfile.getNextBit() == 1, then set cur_node's Character to bfile.getNextByte() and return
 * If bfile.getNextBit() == 0, then initialize cur_node's left and right children, and call readTree() on them
*/
void HuffmanTree::readTree(TreeNode* cur_node, BinaryFileReader& bfile)
{
    if (bfile.hasBits()) {
        bool cur_bit = bfile.getNextBit();
        if (cur_bit == 1) {
            char cur_byte = bfile.getNextByte();
            cur_node->freq = Frequency(cur_byte, 0);
            return;
        } else {
            // cur_bit == 0
            cur_node->left = new TreeNode(0);
            readTree(cur_node->left, bfile);
            cur_node->right = new TreeNode(0);
            readTree(cur_node->right, bfile);
        }
    }
}
