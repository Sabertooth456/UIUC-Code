...
std::vector<Edge> NimLearner::playRandomGame() const {
 ...
 /* Your code goes here! */
  ...
  while(current[3] != '0') {
    int choice = rand() % adjacent.size();
    path.push_back(g_.getEdge(current, adjacent[choice]));
    current = adjacent[choice];
    adjacent = g_.getAdjacent(current);
  }
  return path;
}
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
  ...
  for (Edge e : path) {
    if (e.source[1] == '1') {
      g_.setEdgeWeight(e.source, e.dest, e.getWeight() + weight1);
    } else {
      g_.setEdgeWeight(e.source, e.dest, e.getWeight() + weight2);
    }
  }
}
