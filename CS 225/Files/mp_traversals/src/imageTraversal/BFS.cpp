...
Point BFS::pop() {
  /** @todo [Part 1] */
  if (empty()) {
    std::cout << "BFS pop(): Out of nodes to return" << std::endl;
    return Point();
  }
  Point point = nodes_to_visit.front();
  nodes_to_visit.pop();
  return point;
}
...
Point BFS::peek() const {
  /** @todo [Part 1] */
  if (empty()) {
    // std::cout << "BFS peek(): Out of nodes to return" << std::endl;
    return Point();
  }
