...
class KDTree
{
  ...
  private:
    ...
    /** Helper constructors */
    KDTreeNode* KDTreeConstructor(vector<Point<Dim>>& points, int lowerBound, int upperBound, int d);
    void copy_(KDTreeNode* curNode, KDTreeNode*& parent);

    /** Helper destructor */
    void clear_(KDTreeNode* curNode);

    /** Helper function that uses quickselect to find the median of an unsorted vector list */
    void sort(vector<Point<Dim>>& points, int lowerBound, int upperBound, int k, int dimension);
    int partition(vector<Point<Dim>>& points, int lowerBound, int upperBound, int pivotIndex, int dimension);

    /** Helper functions for findNearestNeighbor */
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode* curNode, int curDim) const;
    double distance(const Point<Dim>& pointA, const Point<Dim>& pointB) const;
};
