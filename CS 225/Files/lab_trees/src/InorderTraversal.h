...
class InorderTraversal : public TreeTraversal<T> {
  ...
  private:
    /**
     * Given a node in the tree, add every node to its left
     * such that when popped, an inorder traversal is simulated.
     * Hint: See lab handout for guidance
     * 
     * @param treeNode The subroot that determines what next nodes
     *        should be added to the traversal
     */	
    void addLeft(typename BinaryTree<T>::Node *& treeNode) {
      if (treeNode != nullptr) {
        stack.push(treeNode);
        addLeft(treeNode->left);
      }
    }
