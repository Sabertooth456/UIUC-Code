...
// void AddDoctor(Doctor& d) { doctors_.push_back(&d); }
...
// bool IsAvailable(TimeSlot time) const { return schedule_.IsAvailable(time); }
...
// void RemoveAppointment(TimeSlot time) { schedule_.MarkAvailable(time); }
